...
PatternEmitter.prototype._emit =
  PatternEmitter.prototype.emit;

PatternEmitter.prototype._addListener =
  PatternEmitter.prototype.addListener;

PatternEmitter.prototype._removeListener =
  PatternEmitter.prototype.removeListener;

PatternEmitter.prototype._removeAllListeners =
  PatternEmitter.prototype.removeAllListeners;
...
PatternEmitter.prototype.emit = function (type) {
  ...
  let m = this._getMatching(type);
  this._events[type] = m.matching;

  this.matches = m.matches;

  this.groups = this._events[type];
 ...
 */
PatternEmitter.prototype.addListener = function (
  type,
  listener
) {
  ...
  PatternEmitter._apply(this, this._addListener, pattern, [
    pattern,
    listener,
  ]);
 ...
 * @throws  {TypeError}      If listener is not a function
 */
PatternEmitter.prototype.removeListener = function (
  type,
  listener
) {
  if (!(type instanceof RegExp)) {
  ...
  if (
    !this._patternEvents[pattern] ||
    !this._patternEvents[pattern].length
  ) {
 ...
 * @returns {PatternEmitter} This instance
 */
PatternEmitter.prototype.removeAllListeners = function (
  type
) {
  if (!(type instanceof RegExp)) {
 ...
 */
PatternEmitter.prototype.patternListeners = function (
  pattern
) {
  if (!(pattern instanceof RegExp)) {
    throw TypeError(
      'pattern must be an instance of RegExp'
    );
  }

  return PatternEmitter._apply(
    this,
    this.listeners,
    pattern,
    [pattern]
 ...
 * @returns {function[]} An array of listeners
 */
PatternEmitter.prototype.matchingListeners = function (
  type
) {
  let m = this._getMatching(type);
  var matching = m.matching;
 ...
 */
PatternEmitter.patternListenerCount = function (
  emitter,
  pattern
) {
  if (!(pattern instanceof RegExp)) {
 ...
 * @returns {int} The number of listeners
 */
PatternEmitter.matchingListenerCount = function (
  emitter,
  type
) {
...
PatternEmitter.prototype._getMatching = function (type) {
  var matching, listeners, matches;
 ...
 */
PatternEmitter._apply = function (
  emitter,
  fn,
  pattern,
  args
  ...
  emitter._events[pattern] =
    emitter._patternEvents[pattern];
